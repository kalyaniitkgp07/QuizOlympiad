...
function _storeRoundsResponse(response) {
  if(response.status) {
...
const Store = React.addons.update(EventEmitter.prototype, { $merge: {
  ...
  dispatcherIndex: Dispatcher.register((payload) => {
    ...
    switch(action.actionType){
      case ActionConstants.FETCH_GAME_ROUNDS:
        _storeRoundsResponse({
          status  : ApiUtils.LOADING_STATE.LOADING,
        });
        break;
      case ActionConstants.FETCH_GAME_ROUNDS_SUCCESS:
        _storeRoundsResponse({
          status  : ApiUtils.LOADING_STATE.SUCCESS,
          result  : action.data,
        });
        break;
      case ActionConstants.FETCH_GAME_ROUNDS_FAILURE:
        _storeRoundsResponse({
