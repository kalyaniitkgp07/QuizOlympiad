...
function _storeGamesResponse(response) {
  if(response.status) {
...
const Store = React.addons.update(EventEmitter.prototype, { $merge: {
  ...
  dispatcherIndex: Dispatcher.register((payload) => {
    ...
    switch(action.actionType){
      case ActionConstants.FETCH_TOURNAMENT_GAMES:
        _storeGamesResponse({
          status  : ApiUtils.LOADING_STATE.LOADING,
        });
        break;
      case ActionConstants.FETCH_TOURNAMENT_GAMES_SUCCESS:
        _storeGamesResponse({
          status  : ApiUtils.LOADING_STATE.SUCCESS,
          result  : action.data,
        });
        break;
      case ActionConstants.FETCH_TOURNAMENT_GAMES_FAILURE:
        _storeGamesResponse({
