...
function _storeTournamentsResponse(response) {
  if(response.status) {
...
const Store = React.addons.update(EventEmitter.prototype, { $merge: {
  ...
  dispatcherIndex: Dispatcher.register((payload) => {
    ...
    switch(action.actionType){
      case ActionConstants.FETCH_ALL_TOURNAMENTS:
        _storeTournamentsResponse({
          status  : ApiUtils.LOADING_STATE.LOADING,
        });
        break;
      case ActionConstants.FETCH_ALL_TOURNAMENTS_SUCCESS:
        _storeTournamentsResponse({
          status  : ApiUtils.LOADING_STATE.SUCCESS,
          result  : action.data,
        });
        break;
      case ActionConstants.FETCH_ALL_TOURNAMENTS_FAILURE:
        _storeTournamentsResponse({
